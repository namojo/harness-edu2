...
| 2026-08-31 | **슬라이드 덱 20장 신규 제작** (1920×1080 PNG + PPTX + PDF) | slides/, docs/slides/ | 사용자 지시 — "전체 페이지를 이미지 슬라이드로, 다이어그램과 전달 방식을 디자인 전문가스럽게". 웹 스크롤 페이지가 아니라 슬라이드 매체로 재구성 |
| 2026-08-31 | 슬라이드 빌드 파이프라인에 **넘침 검사** 편입 | slides/src/build.py | 1920×1080 을 넘으면 렌더가 조용히 잘린다. playwright 로 실제 높이를 재서 렌더 전에 검출 — 3장(06·10·12) 발견해 수정 |
| 2026-08-31 | 전 페이지 네비게이션에 `슬라이드` 추가 | docs/ 10문서 | — |
| 2026-09-01 | **sample.pptx 를 가상 브랜드 제안서로 교체** | slides/sample-src/, workshop/ | 외부 제작물(실브랜드 캠페인 제안서)의 공개 배포 권리가 불확실했다. `codex image_generation` 으로 가상 브랜드 5장을 병렬 생성. 디자인 문법은 계승하고 카테고리는 실습 2 예시와 다르게 했다 — 샘플은 형식만 제공한다는 것이 분명해지도록 |
| 2026-09-01 | 슬라이드 조판 교정 — 제목·설명 **한 줄 자동 맞춤**, 페이지 번호·하단 말꼬리 삭제, 하단 여백 96px + 본문 세로 분배 | slides/src/base.css, build.py | 제목이 불필요하게 2줄이 되고, 하단 말꼬리가 글상자·페이지 번호와 겹치고, 글상자가 슬라이드 밑에 붙는다는 피드백. 폰트를 손으로 맞추면 내용 수정 때마다 어긋나므로 렌더 직전 실제 텍스트 폭을 재서 줄이는 방식으로 자동화 |
...
**슬라이드 덱:** https://namojo.github.io/harness-edu2/slides/ — 20장 · 1920×1080. `slides/src/build.py` 로 재생성. PPTX·PDF 다운로드 제공.
...
**sample.pptx:** **가상 브랜드 '핀치(PINCH)' 반려견 덴탈 츄 '데일리츄' 제안서** 5장 · 1.0MB. `codex image_generation` 으로 병렬 생성 후 투명 배경을 크림에 합성. 외부 제작물을 쓰지 않으므로 **권리 문제 없음**. 재생성 절차는 `slides/sample-src/`.
